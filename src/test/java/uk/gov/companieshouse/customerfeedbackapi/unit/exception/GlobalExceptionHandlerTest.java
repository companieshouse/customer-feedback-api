...
import uk.gov.companieshouse.customerfeedbackapi.exception.GlobalExceptionHandler;
...
class GlobalExceptionHandlerTest {
  ...
  @Captor private ArgumentCaptor<Map<String, Object>> logMapCaptor;
  ...
  void testHandleExceptionReturnsCorrectResponse() {
    ...
    ResponseEntity<Object> entity =
        globalExceptionHandler.handleSendEmailException(new Exception(), webRequest);
  ...
  void testHandleExceptionEncodesException() {
    ...
    try (MockedStatic<ApiLogger> apiLogger = mockStatic(ApiLogger.class)) {
      ...
      globalExceptionHandler.handleSendEmailException(exception, webRequest);

      apiLogger.verify(
          () ->
              ApiLogger.errorContext(
                  eq(REQUEST_ID), eq("exception message \\n"), eq(null), logMapCaptor.capture()),
          times(1));
    }
  }
  ...
  void testHandleExceptionTruncatesException() {
    ...
    try (MockedStatic<ApiLogger> apiLogger = mockStatic(ApiLogger.class)) {
      ...
      apiLogger.verify(
          () ->
              ApiLogger.errorContext(
                  eq(REQUEST_ID), eq("12345678901234567890"), eq(null), logMapCaptor.capture()),
          times(1));
    }
  }
